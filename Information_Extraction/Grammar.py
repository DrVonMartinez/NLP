...
class Grammar:
    ...
    def add_rule(self, rule: None, probability: float = 0, name=None):
        if not name:
            name = rule.__name__
    ...
    def add_rules(self, rule: tuple, probability: tuple[float], name: str):
...
if __name__ == '__main__':
    ...
    pcfg.add_rules(tuple([is_sentence]), tuple([1.0]), name='Sentence')
    pcfg.add_rules((is_noun_phrase_a, is_noun_phrase_b, is_noun_phrase_c), tuple([0.1, 0.2, 0.7]), name='Noun')
    # pcfg.add_rule(is_noun_phrase_a, 0.2)
    # pcfg.add_rule(is_noun_phrase_b, 0.2)
    # pcfg.add_rule(is_noun_phrase_c, 0.7)
    pcfg.add_rules((is_verb_phrase_a, is_verb_phrase_b), tuple([0.6, 0.4]), name='Verb')
    # pcfg.add_rule(is_verb_phrase_a, 0.6)
    # pcfg.add_rule(is_verb_phrase_b, 0.4)
    pcfg.add_rules(tuple([is_prepositional_phrase]), tuple([1.0]), name='Prepositional')
    pcfg.add_terminal_symbol('.')
    for z, p, n in words:
        pcfg.add_non_terminal_symbol(z, p, n)
    print(pcfg.show_rules())
    print(pcfg.show_non_terminal_words())
